...
const Header = () => {
  ...
  return (
    <nav className="sticky top-0 z-40 text-sm bg-white flex flex-col md:flex-row md:items-center md:justify-between p-4 py-2 gap-2 shadow-sm">
      ...
      <div
        className={`flex flex-col md:flex-row md:items-center ${isOpen ? "block" : "hidden md:flex"}`}
      >
        {/* Links */}
        <ul className="flex flex-col md:flex-row md:gap-4 md:pr-6">
          {["/", "/about", "/shoes/men", "/shoes/women", "/shoes/kids"].map(
            (path, index) => {
              const labels = ["Home", "About Us", "Men", "Women", "Kids"];
              return (
                <li
                  key={path}
                  className="hover:bg-slate-100 transition-all duration-300 delay-100 p-2 rounded"
                >
                  <NavLink
                    to={path}
                    onClick={toggleMenu}
